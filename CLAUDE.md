...
- `npm run balance`: botjes spelen het spel zonder browser en geven een tabel met geld, vervoerd, verlopen en game over.
...
- `src/sim/`: de simulatie, zonder DOM. `game.ts` (spelregels en acties), `train.ts`, `routing.ts`, `config.ts` (alle balansgetallen in `BALANCE`), `state.ts`, `layout.ts`, `types.ts`.
...
- `scripts/`: `bots.ts` (botjes en simulatie-runner) en `balance.ts` (de tabel van `npm run balance`).
